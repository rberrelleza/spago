...
import (
	...
	"github.com/nlpodyssey/spago/pkg/mat/internal/mkl/chk"
)
...
func PrintColMajorC(m, n int, data []complex128, nfmtR, nfmtI string) (l string) {
	...
	for i := 0; i < m; i++ {
		...
		for j := 0; j < n; j++ {
			...
			v := data[i+j*m]
			l += fmt.Sprintf(nfmtR, real(v)) + fmt.Sprintf(nfmtI, imag(v)) + "i"
		}
	}
	return
}
...
func PrintColMajorCpy(m, n int, data []complex128, nfmtR, nfmtI string) (l string) {
	...
	for i := 0; i < m; i++ {
		...
		for j := 0; j < n; j++ {
			...
			v := data[i+j*m]
			l += fmt.Sprintf(nfmtR, real(v)) + fmt.Sprintf(nfmtI, imag(v)) + "j"
		}
		l += "],\n"
	}
	l += "], dtype=complex)"
